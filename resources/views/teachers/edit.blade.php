...
<form action="{{ route('teachers.update', ['id' => $teacher['id']]) }}" method="POST" class="space-y-6 border border-[#E5E3DB] bg-white p-8">
    ...
    <div>
        ...
        <select id="gender" name="gender"
            class="w-full border border-[#D9D6CD] bg-[#FCFBF8] px-3.5 py-2.5 text-sm focus:border-[#A16207] focus:bg-white focus:outline-none">
            <option value="L" {{ old('gender', $teacher['gender']) === 'Laki-Laki' ? 'selected' : '' }}>Laki-laki</option>
            <option value="P" {{ old('gender', $teacher['gender']) === 'Perempuan' ? 'selected' : '' }}>Perempuan</option>
        </select>
    </div>
